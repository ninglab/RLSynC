...
def generate_predictions(self, n=10, beam_size=3, indices=None):
...
eval_trainer.validation_data = [x for x in load_synthons_reaction_center_json(args.testdata)
                                if len(x[0].split(".")) == 2]
# print(len(eval_trainer.validation_data))
# task = int(os.environ["SLURM_ARRAY_TASK_ID"])
# eval_trainer.validation_data = eval_trainer.validation_data[(task-1)*32:(task)*32]

results = eval_trainer.evaluate_beam(n=args.n, beam_size=args.k, indices=range(len(eval_trainer.validation_data))) # indices=range((task-1)*32,(task)*32))
