...
class Translator(object):
    ...
    def _fast_translate_batch(self,
        ...
        for step in range(max_length):
            ...
            select_indices = batch_index.view(-1).type(torch.int64)
    ...
    def _translate_batch(self, batch, data):
        ...
        import sys
        sys.stderr.write("\nself.model.encoder.parameters device:")
        sys.stderr.write(str(next(self.model.encoder.parameters()).get_device()))
        sys.stderr.write("\nsrc.device: ")
        sys.stderr.write(str(src.get_device()))
        sys.stderr.write("\n")
        if data_type == 'text':
            _, src_lengths = batch.src
